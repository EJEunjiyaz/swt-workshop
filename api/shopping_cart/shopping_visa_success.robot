...
Suite Setup    Create Session    shopping    https://www.dminer.in.th
...
*** Keywords ***
Search
    ${resp}=    Get Request    shopping    /api/v1/product
...
Product Detail
    ${resp}=    Get Request    shopping    /api/v1/product/2    headers=&{headers}
    Request Should Be Successful    ${resp}
    ...
    ${product_quantity}=    Set Variable    ${resp.json()["quantity"]}
    Set Test Variable    ${product_quantity}
...
Submit Order
    ...
    ${resp}=    Post Request    shopping    /api/v1/order    json=${data}    headers=&{headers}
    Request Should Be Successful    ${resp}
    Should Be Equal As Integers    ${resp.json()["order_id"]}    8004359122
#    Should Be Equal As Numbers    ${resp.json()["total_price"]}    121.95
...
Confirm Payment
    ...
    ${resp}=    Post Request    shopping    /api/v1/confirmPayment    json=${data}    headers=&{headers}
    Request Should Be Successful    ${resp}
